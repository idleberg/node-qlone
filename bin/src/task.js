...
var runTask = function (program) {
    ...
    var defaultTasks = [
        {
            ...
            task: function () {
                return execa.stdout('git', cloneArgs).catch(function (error) {
                    if (error !== '') {
    ...
    if (typeof program.fetch !== 'undefined') {
        var fetchTask = {
            title: "Fetching refs",
            task: function () {
                return execa.stdout('git', ['fetch'], { cwd: dirName }).catch(function (error) {
                    if (error !== '') {
                        throw new Error(error);
                    }
                });
    ...
    if (typeof program.install !== 'undefined') {
        var lookTask = {
